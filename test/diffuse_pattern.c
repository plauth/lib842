...
int main(void)
{
	...
	for (int it = 0; it < 13; it++) {
		...
		for (size_t i = 0; i < BUFFER_SIZE; i++) {
			tmpbuf[i] =
				(uint8_t)((data_buffer[(i + BUFFER_SIZE - 1) %
						       BUFFER_SIZE] +
					   data_buffer[(i + 1) % BUFFER_SIZE]) /
					  3);
		}
		...
		int ret_compress = lib842_compress(data_buffer, BUFFER_SIZE,
						   comp_data, &comp_size);
		if (ret_compress == 0) {
			uint8_t *uncomp_data = (uint8_t *)malloc(BUFFER_SIZE);
			size_t uncomp_size = BUFFER_SIZE;
			int ret_decompress =
				lib842_decompress(comp_data, comp_size,
						  uncomp_data, &uncomp_size);
			if (ret_decompress == 0) {
				if (memcmp(data_buffer, uncomp_data,
					   BUFFER_SIZE) == 0) {
