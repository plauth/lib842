...
int main(int argc, char *argv[])
{
	...
	if (argc != 3 || (impl = test842_get_impl_by_name(argv[1])) == NULL ||
		...
		printf("test_decompress_pattern_halfsmall IMPL PATTERN\n");
		return EXIT_FAILURE;
	}
	...
	uint8_t *out = aligned_alloc(impl->required_alignment, pattern->uncompressed_len / 2);
	size_t olen = pattern->uncompressed_len / 2;
